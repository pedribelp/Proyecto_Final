...
def bubble_sort(arr, key=lambda x: x, reverse=False):
    ...
    for i in range(n):
        # si no hay intercambios, ya está ordenado
        swapped = False
    ...
    print(f"Bubble Sort: {comparaciones} comparaciones, {intercambios} intercambios")
    return resultado
...
class SistemaFigurasApp:
    ...
    def visualizar_figuras(self, figuras_a_mostrar=None):
        """Visualiza las figuras con un tamaño fijo"""
        ...
        canvas_frame = ttk.Frame(self.visualizacion_frame)
        ...
        # Crear un canvas que contendrá la figura de matplotlib
        ...
        # Frame interior para la figura
        figura_frame = ttk.Frame(canvas)
        
        # Dibujar las figuras con tamaño fijo
        num_figuras = len(figuras_a_mostrar)
        cols = 4  # Número fijo de columnas
        filas = (num_figuras + cols - 1) // cols
        ...
        # Asegurar que axs sea un array
        axs = np.array(axs).reshape(-1) if num_figuras > 1 else np.array([axs])
        ...
        # Crear canvas de matplotlib en el frame
        mpl_canvas = FigureCanvasTkAgg(fig, master=figura_frame)
        mpl_widget = mpl_canvas.get_tk_widget()
        mpl_widget.pack()
        
        # Mostrar la ventana de matplotlib en el canvas de Tkinter
        canvas.create_window((0, 0), window=figura_frame, anchor=tk.NW)
        
        # Actualizar regiones de desplazamiento
        figura_frame.update_idletasks()
        canvas.config(scrollregion=canvas.bbox(tk.ALL))
        
        mpl_canvas.draw()

        def redimensionar_visualizacion(self, event):
            """Maneja el redimensionamiento de la ventana con un retraso mayor"""
            # Cancelar cualquier redibujado pendiente
            if hasattr(self, '_job'):
                self.visualizacion_frame.after_cancel(self._job)
            # Esperar medio segundo antes de actualizar (500ms en lugar de 200ms)
            self._job = self.visualizacion_frame.after(500, self.actualizar_visualizacion)

        def actualizar_visualizacion(self):
            """Actualiza la visualización después del redimensionamiento"""
            if hasattr(self, 'figuras_visualizadas'):
                # Limpiar el frame de visualización
                for widget in self.visualizacion_frame.winfo_children():
                    widget.destroy()
                
                # Crear un nuevo frame contenedor
                container_frame = ttk.Frame(self.visualizacion_frame)
                container_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
                
                # Recrear la visualización
                self.crear_visualizacion(container_frame, self.figuras_visualizadas)
    ...
    def buscar_por_area(self):
        ...
        try:
            ...
            figuras_ordenadas = bubble_sort(self.figuras, key=lambda x: x.calcular_area())
