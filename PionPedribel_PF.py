...
def bubble_sort_iterative(arr, key=lambda x: x, reverse=False):
    """
    Versión iterativa del algoritmo Bubble Sort usando bucles anidados.
    Ordena una lista y devuelve una copia ordenada sin modificar la original.
    """
    n = len(arr)
    comparaciones = 0
    intercambios = 0
...
def bubble_sort_recursive(arr, key=lambda x: x, reverse=False):
    """
    Versión recursiva del algoritmo Bubble Sort.
    Ordena una lista y devuelve una copia ordenada sin modificar la original.
    """
    def recursive_sort(resultado, n, comparaciones_intercambios):
        if n == 1:
            return comparaciones_intercambios
        
        swapped = False
        for i in range(n - 1):
            comparaciones_intercambios[0] += 1
            ...
            if condicion:
                ...
                swapped = True
                comparaciones_intercambios[1] += 1
        
        if not swapped:
            return comparaciones_intercambios
        
        return recursive_sort(resultado, n - 1, comparaciones_intercambios)
    
    resultado = arr.copy()
    comparaciones_intercambios = [0, 0]  # [comparaciones, intercambios]
    recursive_sort(resultado, len(resultado), comparaciones_intercambios)
    print(f"Bubble Sort Recursivo: {comparaciones_intercambios[0]} comparaciones, {comparaciones_intercambios[1]} intercambios")
    return resultado
...
class SistemaFigurasApp:
    ...
    def ordenar_por_area(self):
        ...
        self.figuras = bubble_sort_iterative(self.figuras, key=lambda x: x.calcular_area())
    ...
    def ordenar_por_perimetro(self):
        ...
        # Usar el algoritmo de ordenamiento bubble sort
        self.figuras = bubble_sort_recursive(self.figuras, key=lambda x: x.calcular_perimetro(), reverse=True)
    ...
    def buscar_por_area(self):
        ...
        try:
            ...
            figuras_ordenadas = bubble_sort_iterative(self.figuras, key=lambda x: x.calcular_area())
